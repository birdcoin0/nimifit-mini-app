...
const callCreatePost = (p) => callApi("createPost", p);
const callToggleLike = (p) => callApi("toggleLike", p);
const callAddComment = (p) => callApi("addComment", p);
...
export default function TouchGrass({ user, openAuth, setAccount }) {
  ...
  async function publish(event) {
    ...
    setError("");
    try {
      const result = await callCreatePost({ caption: caption.trim(), imageData });
  ...
  async function like(post) {
    if (!user) return openAuth();
    try {
      const result = await callToggleLike({ postId: post.id });
      setAccount((current) => ({ ...current, tickets: result.data.tickets }));
      await refresh();
    } catch (err) {
      setError(err.message || "Could not like this moment.");
    }
  }
...
<<<<<<< HEAD
    try {
      await callAddComment({ postId: post.id, text: text.trim() });
      await refresh();
    } catch (err) {
  ...
  return (
    <main className="page-wrap">
      <header className="topbar">
        <div>
          <span className="eyebrow">Outside counts too</span>
          <h1>Touch Grass</h1>
        </div>
        <span className="leaf-mark">✦</span>
      </header>
      <section className="hero-panel grass-hero">
        <div>
          <span className="eyebrow">Real moments, not perfect ones</span>
          <h2>Proof you <em>showed up.</em></h2>
          <p>Share a walk, a workout, a meal outside, or anything that got you away from the screen.</p>
        </div>
      </section>
      <form className="post-composer" onSubmit={publish}>
        <span className="eyebrow">Share a moment</span>
        <textarea
          ...
          maxLength={280}
        />
        {imageData && <img className="composer-image" src={imageData} alt="Preview" />}
        <div className="button-row">
          ...
          <input
            ...
            accept="image/*"
            onChange={async (e) => {
              if (e.target.files?.[0]) setImageData(await compressImage(e.target.files[0]));
            }}
          />
          <button className="primary-button" disabled={busy}>
            {busy ? "Posting…" : "Post · +50 🎟"}
          </button>
        </div>
      ...
      <div className="feed-head">
        ...
        <span className="muted">{posts.length} recent</span>
      </div>
      <div className="feed">
        {posts.length === 0 ? (
          <div className="empty-state">No moments yet. Be the first one outside.</div>
        ) : (
          posts.map((post) => (
            <article className="post-card" key={post.id}>
              {post.imageData && <img src={post.imageData} alt="Community moment" />}
              <div className="post-body">
                <div className="post-meta">
                  <strong>{post.displayName || "Anonymous"}</strong>
                  <span>{post.location || "Somewhere outside"}</span>
                </div>
                {post.caption && <p>{post.caption}</p>}
                <div className="post-actions">
                  <button onClick={() => like(post)}>♡ {post.likes?.length || 0}</button>
                  <button onClick={() => comment(post)}>Comment {post.comments?.length || 0}</button>
                  <span>+50 for posting</span>
                </div>
              </div>
            </article>
          ))
